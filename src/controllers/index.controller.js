...
const createUser = async(req,res)=>{
    ...
    try{
        const {correo,name,pass,suscripcion,admin,boolean} = req.body
        //const id = 'DEFAULT'
        const rondas = 10
        ...
        if(prof.rowCount ===0){
            const response = await pool.query('insert into usuarios(correo,nombre,contraseña,estado,suscripcion, admin_user,is_admin) values($1,$2,$3,$4,$5,$6,$7)',[correo,name,haspass,false,suscripcion,admin,boolean])
...
const createPelis = async (req, res) => {
    try{
        const {nombre, categoria, fecha_estreno, duracion, link, imagen,admin,boolean} = req.body

        const prof = await pool.query('SELECT * FROM peliculas_series WHERE nombre = $1',[nombre])
        
        console.log(nombre, categoria, fecha_estreno, duracion, link, imagen, admin)
        console.log(prof.rows)
         
        if(prof.rowCount ===0){
            const response = await pool.query('insert into peliculas_series(nombre,categoria,fecha_estreno,duracion,link,imagen,admin_user,is_admin) values($1,$2,$3,$4,$5,$6,$7,$8)',
            [nombre, 
            categoria,
            fecha_estreno,
            duracion,
...
const createProfile = async (req, res)=> {
    try{
        const {id_usuario, name, admin,boolean} = req.body
        console.log(name)
        const response = await pool.query('insert into perfil(id_usuario, nombre, viendo, activo, admin_user,is_admin) values ($1,$2,$3,$4,$5,$6)',[id_usuario, name, false, true,admin,boolean])
        console.log(response)
...
const updateProf = async (req, res) => {
    try{
        ...
        console.log(boolean,id, sus, num)
        const newSus = await pool.query('UPDATE usuarios SET suscripcion = $1,admin_user = $2, is_admin = $3 WHERE id = $4',[sus,admin,boolean,id])
        const porfil = await pool.query('select * from perfil where id_usuario =  $1 limit $2',[id,num])
        const setFalse = await pool.query('UPDATE perfil SET activo = false,admin_user = $1,is_admin=$2 WHERE id_usuario =$3',[admin,boolean,id])
        //console.log(newSus)
        for(const pepe in porfil.rows){
            const setTreu = await pool.query('UPDATE perfil SET activo = true,admin_user=$1,is_admin=$2 WHERE id_perfil =$3', [admin,boolean,porfil.rows[pepe].id_perfil])
...
const getAdmin = async (req,res) => {
    try{
        const response = await pool.query('Select * from administrador order by nombre')
        res.status(200).json(response.rows)
    }catch (e){
        console.log("ERROR")
...
const updateAdminUser = async (req, res) => {
    try{
        ...
        const id = req.params.id
        const {valor, estado,tabla,admin,boolean} = req.body
        console.log(valor, estado,tabla,id,admin,boolean)
        switch(tabla){
            case 'usuarios':
                console.log('Usuarios')
                switch(valor){
                    case 'nombre' :
                        const responseN = await pool.query('update usuarios set nombre = $1, admin_user = $2,is_admin =$3 where id = $4', [estado,admin,boolean, id])
                        console.log(responseN)
                        res.json('User Updated')
                        break;
                    case 'correo' :
                        const responseC = await pool.query('update usuarios set correo = $1, admin_user = $2,is_admin = $3 where id = $4', [estado,admin,boolean, id])
                    ...
                    case 'estado' :
                        ...
                        console.log(est)
                        const responseE = await pool.query('update usuarios set estado = $1, admin_user = $2,is_admin =$3 where id = $4', [estado,admin,boolean,id])
                        console.log(responseE.rows)
                        res.json('User Updated')
                        break;
                    case 'suscripcion' :
                        const responseS = await pool.query('update usuarios set suscripcion = $1, admin_user = $2, is_admin =$3 where id = $4', [estado,admin,boolean, id])
            ...
            case 'peliculas_series':
                ...
                switch(valor){
                    case 'nombre' :
                        const responseN = await pool.query('update peliculas_series set nombre = $1, admin_user = $2, is_admin =$3 where codigo= $4',  [estado,admin,boolean, id])
                        console.log(responseN)
                        res.json('Pelicula Updated')
                        break;
                    case 'categoria' :
                        const responseC = await pool.query('update peliculas_series set categoria = $1, admin_user = $2, is_admin =$3 where codigo= $4', [estado,admin,boolean, id])
                        console.log(responseC)
                        res.json('Pelicula Updated')
                        break;
                    case 'fecha_estreno' :
                        const responseE = await pool.query('update peliculas_series set fecha_estreno = $1, admin_user = $2, is_admin =$3 where codigo= $4',  [estado,admin,boolean, id])
                        console.log(responseE)
                        res.json('Pelicula Updated')
                        break;
                    case 'duracion' :
                        console.log('duracion')
                        const responseD = await pool.query('update peliculas_series set duracion = $1, admin_user = $2, is_admin =$3 where codigo= $4',  [estado,admin,boolean, id])
                        console.log(responseD)
                        res.json('Pelicula Updated')
                        break;
                    case 'link' :
                        const responseL = await pool.query('update peliculas_series set link = $1, admin_user = $2, is_admin =$3 where codigo= $4', [estado,admin,boolean, id])
                        console.log(responseL)
                        res.json('Pelicula Updated')
                        break;
                    case 'imagen' :
                        const responseIM = await pool.query('update peliculas_series set imagen = $1, admin_user = $2,is_admin=$3 where codigo= $4', [estado,admin,boolean, id])
            ...
            case 'actor':
                ...
                switch(valor){
                    case 'nombre' :
                        const responseN = await pool.query('update actor set nombre = $1, admin_user = $2, is_admin= $3 where id = $4', [estado,admin,boolean, id])
                        console.log(responseN)
                        res.json('actor Updated')
                        break;
                    case 'apellido' :
                        const responseC = await pool.query('update actor set apellido = $1, admin_user = $2, is_admin= $3 where id = $4', [estado,admin,boolean, id])
                        console.log(responseC)
                        res.json('actor Updated')
                        break;
                }
            ...
            case 'anuncio':
                ...
                switch(valor){
                    case 'contenido' :
                        const responseN = await pool.query('update anuncio set contenido = $1, admin_user = $2, is_admin = $3 where contenido = $4', [estado,admin,boolean, id])
                        console.log(responseN)
                        res.json('anuncio Updated')
                        break;
                    case 'link' :
                        const responseC = await pool.query('update anuncio set link = $1, admin_user = $2, is_admin = $3 where contenido = $4', [estado,admin,boolean, id])
                        console.log(responseC)
                        res.json('anuncio Updated')
                        break;
                }
            ...
            case 'anunciantes':
                ...
                switch(valor){
                    case 'anunciante' :
                        const responseN = await pool.query('update anunciantes set anunciante = $1, admin_user = $2, is_admin = $3 where id = $4', [estado,admin,boolean, id])
                        console.log(responseN)
...
const deleteAdminUser = async (req, res) => {
    try{
        const id = req.params.id
        const {tabla,admin} = req.body
        console.log(tabla,id)
        switch(tabla){
            case 'peliculas_series':
                const responseP = await pool.query('Select Delete_PS($1,$2)',[admin,req.params.id])
                res.json(`Pelicula ${req.params.id} eliminado de BD`)
                break;
            case 'anuncio':
                const responseAN = await pool.query('Select Delete_An($1,$2)',[admin,req.params.id])
                res.json(`Anuncio ${req.params.id} eliminado de BD`)
                break;
            case 'anunciantes':
                const responseAT = await pool.query('Select Delete_Anu($1,$2)',[admin,req.params.id])
                res.json(`Pelicula ${req.params.id} eliminado de BD`)
                break;
            case 'actor':
                const responseAC = await pool.query('Select Delete_Ac($1,$2)',[adminm,req.params.id])
...
const createAnunciante = async (req,res) => {
    try{
        const {anunciante,admin,boolean} = req.body
        console.log(anunciante)
        const response = await pool.query('insert into anunciantes(anunciante,admin_user,is_admin) values ($1,$2,$3)',[anunciante,admin,boolean])
        console.log(response)
...
const createAnuncio = async (req,res) => {
    try{
        ...
        const {anunciante, contenido, link,admin,boolean} = req.body
        ...
        const response = await pool.query('insert into anuncio(id_anunciante, contenido, link,admin_user,is_admin) values ($1, $2, $3, $4, $5)',[id, contenido, link,admin,boolean])
        console.log(response)
...
const getReporte7 = async (req,res) => {
    try{
        const response = await pool.query('select * from Reporte7')
        res.status(202).json(response.rows)
    }catch (e){
        console.log("ERROR")
...
const getReporte6 = async (req, res) => {
    try{
        ...
        try {
            ...
            const queryText = 'Select * from Reporte6($1,$2);'
            const rest = await client.query(queryText, ['a',mes])
            const response = await client.query('fetch all from a;')
...
const getReporte8 = async (req, res) => {
    try{
        ...
        try {
            await client.query('BEGIN')
            const queryText = 'Select * from Reporte8($1,$2,$3);'
            const rest = await client.query(queryText, ['a',fec1,fec2])
            const response = await client.query('fetch all from a;')
            await client.query('COMMIT')
            res.status(202).json(response.rows)
          } catch (e) {
...
const getReporte9 = async (req, res) => {
    try{
        ...
        try {
            await client.query('BEGIN')
            const queryText = 'Select * from Reporte9($1,$2,$3);'
            const rest = await client.query(queryText, ['a',fec1,fec2])
            const response = await client.query('fetch all from a;')
            await client.query('COMMIT')
            res.status(202).json(response.rows)
...
const createActores = async (req, res) => {
    try{
        const {nombre, apellido,admin,boolean} = req.body
        ...
        if(prof.rowCount ===0){
            const response = await pool.query('insert into actor(nombre,apellido,admin_user,is_admin) values($1,$2,$3,$4)',[nombre,apellido,admin,boolean])
...
const Simulacion = async (req, res) => {
    try{
        ...
        try {
            await client.query('BEGIN')
            const queryText = 'Select * from Simulation($1,$2);'
            const res = await client.query(queryText, [fecha,cantidad])
            await client.query('COMMIT')
            client.release()
          } catch (e) {
            await client.query('ROLLBACK')
...
const createSearch = async (req, res) => {
    try{
        ...
        const response = await pool.query('insert into busqueda values($1)',[contenido])
        
        console.log(response.rows)
        return res.json({
            message:'Agregada la busqueda',
        })
